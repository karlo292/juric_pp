...
impl ArchLinux {
    ...
    /// Scrape package page to get repository link
    fn scrape_package(&self, stdout: &mut Stdout, url: &String) {
        ...
        for _match in document.select(&selector) {
            ...
            if element.starts_with("Source") { /* Source Files, a.k.a repo with files to download) */
                ...
                self.download(stdout, &opt_2.as_ref().unwrap().to_owned().to_string());
            }
        }
    }
    
    /// Scrape page with all searches
    fn scrape_matches(&self, stdout: &mut Stdout, package: &String) {
        ...
        let mut pkg_url: String = self.pkg_url.clone();
...
impl AUR {
    /// Install package by running `makepkg -si`; function for AUR & Arch Linux structs
    ...
    /// Scrape AUR page, then generate repository link to download PKGBUILD
    fn scrape(&self, stdout: &mut Stdout, package: &String) {
        ...
        self.download(stdout, &pkg_url, package);
    }
}

/// Start scraping :)
pub fn scrape(stdout: &mut Stdout, os: &String, package: &String) {
    match os.as_str() {
        "arch" => {
            let aur = AUR {
                base_url: format!("https://aur.archlinux.org/packages?O=0&SeB=nd&K={}&outdated=&SB=p&SO=d&PP=50&submit=Go", &package),
                pkg_url: "https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=".to_string(),
            };

            let arch_linux = ArchLinux {
                aur: aur,

                base_url: format!("https://archlinux.org/packages/?sort=&q={}", &package),
                pkg_url: "https://archlinux.org/packages/".to_string(),
            };

            arch_linux.scrape_matches(stdout, package);
        },
